...
class Game
  ...
  def turn
    ...
    #demande au joueur 1 se qu'il joue
    while gameover == 0  
      if player_1_turn  #au tour du joueur 1
      ...
      win_blocks = [[1,2,3],[4,5,6],[7,8,9],[1,4,7],[2,5,8],[3,6,9],[1,5,9],[3,5,7]] # listing des cas possibles pour gagner.
      win_blocks.each {|b| 
        # print b - @joueur_1.list_case_p
        if ((b - @joueur_1.list_case_p).empty?)   #joueur 1 à gagné?
          gameover = 1
          # if player_1_turn 
        ...
        elsif ((b - @joueur_2.list_case_p).empty?)  #joueur 2 à gagné?
            gameover = 1
            if player_1_turn 
    ...
    puts "Une autre partie? 1: Oui/ 0: Non" #propose une nouvelle partie aux joueurs
    print ">"
